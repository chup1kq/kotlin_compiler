#ifndef KOTLIN_COMPILER_CLASSGENERATION_H
#define KOTLIN_COMPILER_CLASSGENERATION_H
#include <cstdint>
#include <fstream>

#include "../semantic/tables/class/ClassTableElement.h"

class ClassTable;
using namespace std;

enum AccessFlags {
    F_ACC_PUBLIC = 0x0001,
    F_ACC_PRIVATE = 0x0002,
    F_ACC_PROTECTED = 0x0004,
    F_ACC_FINAL = 0x0010,
    F_ACC_SUPER = 0x0020,
};

class ClassGeneration {
private:
    // Основные данные
    ClassTable* m_classes;
    string m_filename;                      // "MyProgram.class"

    // Файловые структуры
    ofstream m_out;                  // Бинарный поток
    vector<uint8_t> m_buffer;        // Финальный .class

    // JVM секции
    //ConstantTable* m_constants;             // Пул констант
    //vector<uint8_t> m_fields;               // Поля (пока пустые)
    //vector<uint8_t> m_methods;              // Методы класса

    // JVM метаданные
    //uint16_t m_thisClassIndex;            // #15 this_class
    //uint16_t m_superClassIndex;           // #7 java/lang/Object
    //uint16_t m_accessFlags;               // 0x0021 ACC_PUBLIC

    void writeU2(uint16_t value);
    void writeU4(uint32_t value);
    void generate(ClassTableElement* cls);

    void writeU2Local(std::vector<uint8_t>& buffer, uint16_t value);
    void writeU4Local(std::vector<uint8_t>& buffer, uint32_t value);

    size_t m_headerPos;

public:
    ClassGeneration(ClassTable* cls);

    void generateClassFile(ClassTableElement* cls);
    void generateAllClassesFiles();

    // Секции .class файла
    void writeHeader(ClassTableElement* cls);           // Magic + version
    void writeClassInfo(ClassTableElement* cls);        // this_class, super_class
    void writeFields(ClassTableElement* cls);           // Поля класса
    void writeToFile();           // Запись

    std::vector<uint8_t> generateMainMethod(ClassTableElement* elem, MethodTableElement* main);
    std::vector<uint8_t> generateMethod(ClassTableElement* elem, MethodTableElement *method);
    std::vector<uint8_t> generateInitMethod(ClassTableElement* cls, MethodTableElement* method);

    int findClassConstantIndex(ClassTableElement* cls, const std::string& className);
};

#endif //KOTLIN_COMPILER_CLASSGENERATION_H